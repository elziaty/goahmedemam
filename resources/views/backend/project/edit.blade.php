                            ...
                            <div class="row"> 
                                    @if(isSuperadmin())
                                        <div class="col-6 mt-3">
                                            ...
                                            <select class=" form-control form--control select2" name="business_id" id="business_id" data-url="{{ route('project.business.branches') }}">
                                                <option  disabled selected>{{ __('select') }} {{ __('business') }}</option>
                                                @foreach ($businesses as $business )
                                                    <option  value="{{ $business->id }}" @if($project->business_id == $business->id) selected @endif>{{  @$business->business_name }}</option>
                                                @endforeach
                                            </select>
                                            @error('business_id')
                                            ...
                                            <select name="branch_id" id="branch_id" class="select2 form-control form--control">
                                                <option disabled selected> {{ __('select') }} {{ __('branch') }} </option> 
                                                    @foreach ($branches as $branch)
                                                        <option  value="{{ $branch->id }}" @if(old('branch_id',$project->branch_id) == $branch->id) selected @endif>{{ $branch->name }}</option>
                                                    @endforeach 
                                            </select>
                                            @error('branch_id')
                                                <p class="text-danger pt-2">{{ $message }}</p>
                                    ...
                                    <div class="col-6 mt-3">
                                        ...
                                        <input type="text" name="title" class="form-control form--control" id="title" value="{{ old('title',$project->title) }}">
                                        @error('title')
                                            <p class="text-danger pt-2">{{ $message }}</p>
                                        @enderror
                                    </div>
                                    <div class="col-6 mt-3">
                                        ...
                                        <input type="text" name="date" class="form-control form--control datepicker" id="date" value="{{ old('date',$project->date) }}" readonly>
                                        @error('date')
                                            <p class="text-danger pt-2">{{ $message }}</p>
                                        @enderror
                                    </div>
                                    ...
                                    <div class="col-12 mt-3">
                                        ...
                                        <textarea  name="description" class="form-control form--control " id="description" >{{ old('description',$project->description) }}</textarea>
                                        @error('description')
                                            <p class="text-danger pt-2">{{ $message }}</p>
                                        @enderror
                                    </div>
