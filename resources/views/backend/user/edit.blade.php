            ...
            <div class="row g-4">
                <div class="col-xl-12">
                    <div class="dashboard--widget ">
                        ...
                        <form action="{{ route('user.update',['id'=>$user->id,'update'=>true]) }}" method="post" enctype="multipart/form-data">
                            @csrf
                            @method('put')
                            ...
                            <div class="row mt-3">
                                <div class="col-lg-6">
                                    ...
                                    <input type="text" name="name" class="form-control form--control" id="name" value="{{ old('name',$user->name) }}">
                                    @error('name')
                                ...
                                <div class="col-lg-6">
                                    ...
                                    <input type="email" name="email" class="form-control form--control" id="email" value="{{ old('email',$user->email) }}">
                                    @error('email')
                                        <p class="text-danger pt-2">{{ $message }}</p>
                                    @enderror
                                </div>
                                        ...
                                        @foreach (\Config::get('pos_default.user_type') as $key=>$usertype)
                                            @if(isSuperadmin())
                                                @if($usertype != \App\Enums\UserType::USER)
                                                    <option value="{{ $usertype }}" {{ old('user_type',$user->user_type) == $usertype ? 'selected':'' }}>{{ @user_type_text($usertype) }}</option>
                                                @endif
                                            @elseif(isUser())
                                                @if($usertype == \App\Enums\UserType::USER)
                                                    <option value="{{ $usertype }}" {{ old('user_type',$user->user_type) == $usertype ? 'selected':'' }}>{{ @user_type_text($usertype) }}</option>
                                        ...
                                        <div class="col-sm-5">
                                            <div class="pt-25">
                                                <div class="d-flex mt-3">
                                                    ...
                                                    <input type="checkbox" class="status" id="status" switch="success" name="status" {{ $user->status == App\Enums\Status::ACTIVE ? 'checked' : '' }} >
                                ...
                                @if(isSuperadmin() == true && $user->business_id)
                                    <div class="col-lg-6 mt-3">
                                        ...
                                        <select class="form-control form--control select2" name="business_id" id="business_id" data-url="{{ route('user.business.branch.fetch') }}">
                                            ...
                                            @foreach ($businesses as $business)
                                                <option value="{{ $business->id }}" {{ old('business_id',$user->business_id) == $business->id ? 'selected':'' }}>{{ $business->business_name }}</option>
                                            @endforeach
                                        </select>
                                        @error('business_id')
                                            <p class="text-danger pt-2">{{ $message }}</p>
                                ...
                                <div class="col-lg-6 mt-3">
                                    ...
                                    <input type="text" name="phone" class="form-control form--control" id="phone" value="{{ old('phone',$user->phone) }}">
                                    @error('phone')
                                        <p class="text-danger pt-2">{{ $message }}</p>
                                    @enderror
                                </div>
                                ...
                                <div class="col-lg-6 text-center mt-3">
                                    ...
                                    <div class="pupload mb-5">
                                        <div class="thumb">
                                            <img class="pu-img" src="{{$user->image }}" alt="clients">
                                        </div>
                                ...
                                <div class="col-12">
                                    <div class="row">
                                        <div class="col-lg-6 mt-3">
                                            ...
                                            <textarea  name="address" class="form-control form--control " id="address" >{{ old('address',$user->address) }}</textarea>
                                            @error('address')
                                                <p class="text-danger pt-2">{{ $message }}</p>
                                            @enderror
                                        </div>
                                        ...
                                        <div class="col-lg-6 mt-3">
                                            ...
                                            <textarea  name="about" class="form-control form--control" id="about" >{{ old('about',$user->about) }}</textarea>
                                            @error('about')
                                                <p class="text-danger pt-2">{{ $message }}</p>
                                            @enderror
                                        </div>
