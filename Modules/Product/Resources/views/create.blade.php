                            ...
                            <div class="row mt-3">
                                ...
                                <div class="col-lg-6 mt-3">
                                    ...
                                    <select class="form-control form--control select2" name="barcode_type">
                                        @foreach (\Config::get('pos_default.barcode_types') as $key=>$type )
                                            <option value="{{ $key }}" @if(old('barcode_type',@$business->barcode_type) == $key) selected @endif>{{ __($type) }}</option>
                                        @endforeach
                                    </select>
                                    @error('barcode_type')
                                        <p class="text-danger pt-2">{{ $message }}</p>
                                    ...
                                    <select class="form-control form--control select2" name="warranty_id" id="warranty" >
                                        <option selected disabled>   {{ __('select') }} {{ __('warranty') }} </option>  
                                            @foreach ($warranties as $warranty )
                                                <option value="{{ $warranty->id }}" @if(old('warranty_id') == $warranty->id) selected @endif>{{ @$warranty->name }}</option>
                                            @endforeach 
                                    </select>
                                    @error('warranty_id')
                                        <p class="text-danger pt-2">{{ $message }}</p>
                                ...
                                <div class="col-lg-6 mt-3">
                                    ...
                                    <select class="form-control form--control select2" name="branches[]" id="branch" multiple> 
                                        @foreach ($branches as $branch) 
                                            <option value="{{ $branch->id }}" @if(old('branches') && in_array($branch->id,old('branches',[]))) selected @endif>
                                                {{ $branch->name }}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="col-lg-6 mt-3">
                                    ...
                                    <select class="form-control form--control select2" name="variation_id" id="variation_id" data-url="{{ route('product.variation.values') }}"> 
                                        ...
                                        @foreach ($variations as $variation)
                                            <option value="{{ $variation->id }}" @if(old('variation_id') == $variation->id) selected @endif>{{ $variation->name }}</option>
                                        @endforeach
                                    </select>
                                    @error('variation_id')
                                        <p class="text-danger pt-2">{{ $message }}</p>
                                ...
                                <div class="col-lg-6 mt-3">
                                    ...
                                    <select class="form-control form--control select2" name="variation_values[]" id="variation_values" multiple>
                                        @if(old('variation_values') && !blank(old('variation_values')))
                                           @foreach (old('variation_values') as $value)
                                ...
                                <div class="col-lg-2 mt-3">
                                    ...
                                    <div class="d-flex mt-3">
                                        <input type="checkbox" name="enable_stock" id="manage_stock" @if(old('enable_stock')) checked @endif/>
                                        <label for="manage_stock" class="form-label manage_stock_label">{{ __('manage_stock') }} </label>
                                    </div> 
                                </div>  

                                <div class="col-lg-4 mt-3 alert_quantity alert_quantity_toggle @if(old('enable_stock')) d-block @endif">
                                    <label for="alert_quantity" class="form-label">{{ __('alert_quantity') }} <span class="text-danger">*</span> </label>
                                     <input type="text" class="form-control form--control" value="{{ old('alert_quantity') }}" name="alert_quantity"/>
                                ...
                                <div class="col-lg-4 mt-3  ">
                                    <label for="quantity" class="form-label">{{ __('quantity') }} <span class="text-danger">*</span> </label>
                                     <input type="text" class="form-control form--control" value="{{ old('quantity') }}" name="quantity"/>
                                    @error('quantity')
                                        <p class="text-danger pt-2">{{ $message }}</p>
                                    @enderror
                                </div>  
                                ...
                                <div class="col-lg-4 mt-3  ">
                                    <label for="purchese_price" class="form-label">{{ __('default_purchase_price') }}<span class="text-danger">*</span> </label>
                                     <input type="text" class="form-control form--control" name="default_purchase_price" id="purchese_price" value="{{ old('default_purchase_price') }}" />
                                ...
                                <div class="col-lg-4 mt-3  ">
                                    ...
                                    <input type="text" class="form-control form--control" value="{{ old('profit_percent',@$business->default_profit_percent) }}" name="profit_percent"  id="profit_percent"/>
                                ...
                                <div class="col-lg-4 mt-3  ">
                                    ...
                                    <input type="text" class="form-control form--control" value="{{ old('selling_price') }}" name="selling_price"  id="selling_price"/>
                                    @error('selling_price')
                                        <p class="text-danger pt-2">{{ $message }}</p>
                                    @enderror
                                </div>  
                                ...
                                <div class="col-lg-12 mt-3">
                                    ...
                                    <textarea class="form-control form--control" name="description" id="description" placeholder="{{ __('enter_description') }}" rows="5">{{ old('description') }}</textarea> 
                                    @error('description')
                                        <p class="text-danger pt-2">{{ $message }}</p>
                                    @enderror
                                </div>  
...
@endsection
@push('scripts')
<script src="{{ static_asset('backend') }}/js/product/create.js"></script> 
@endpush