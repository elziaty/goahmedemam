...
@section('title',__('sale_list'))
@section('breadcrumb')
    <div class="breadcrumb-area">
        <h5 class="title text--base m-0">{{ __('sales') }}</h5>
        <ul class="breadcrumb"> 
...
@section('maincontent') 
                    ...
                    <div class="dashboard--widget ">
                        <h4 class="card-title overflow-hidden"> 
                            @if(hasPermission('sale_create'))
                                <a  href="{{ route('sale.create') }}" class="btn btn-primary float-right  "  data-bs-toggle="tooltip" title="{{ __('levels.add') }}"
    ...
    <input type="hidden" id="get-sale-list" data-url="{{ route('sale.get.all') }}"/>
@endsection
@push('scripts') 
<script src="{{ static_asset('backend') }}/js/bootstrap.bundle2.min.js" ></script>  
<script src="{{ static_asset('backend') }}/js/sell/sale/sale_table.js" ></script>  
@endpush
