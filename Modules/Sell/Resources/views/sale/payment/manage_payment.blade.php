        ...
        <table class="table table-striped table-hover">
            <thead>
                <tr class="border-bottom bg-primary text-white head align-middle">
                    ...
                    @if(hasPermission('sale_update_payment') || hasPermission('sale_delete_payment')) 
                    <th>{{ __('action') }} </th> 
                    @endif
                </tr>
            </thead>
                    ...
                    <tr>
                        ...
                        <td data-title="{{ __('action') }}">
                            @if(hasPermission('sale_update_payment')) 
                                <a href="#"  class="edit_payment px-3" data-url="{{ route('sale.edit.payment',$payment->id) }}"><i class="fa fa-pen"></i></a>
                            @endif
                            @if(hasPermission('sale_delete_payment')) 
                                <a href="{{ route('sale.delete.payment',$payment->id) }}"  class="px-3"  ><i class="fa fa-trash"></i></a>
                            @endif
                        </td>
                        @endif 
                    </tr>
...
<div id="payment_form"> 
    ...
    <form action="{{ route('sale.add.payment',['sale_id'=>$sale->id]) }}" method="post">
        @csrf
        <div class="row">   
