...
@section('maincontent')
                ...
                <div class="col-xl-12">
                    <div class="dashboard--widget ">
                        <h4 class="card-title overflow-hidden"> 
                            <a href="{{ route('sale.index') }}" class="btn btn-sm btn-primary float-right" data-bs-toggle="tooltip" title="{{ __('back') }}"
                            ...
                            <div class="row mt-3">   
                                ...
                                <div class="col-lg-6 mt-3">
                                    <div class="ui-widget">
                                        ...
                                        <input type="text" class="form-control form--control" id="product_find" placeholder="{{ __('enter_product_info') }}" data-url="{{ route('sale.variation.location.find') }}"/> 
                                    </div>
                                        ...
                                        <table class="table table-striped table-hover">
                                            ...
                                            <tbody id="sale_item_body"> 
                                                @if (old('variation_locations'))
                                                   @foreach (old('variation_locations') as $variation_location) 
                                                        @include('sell::sale.variation_location_item_old',['item'=>$variation_location])
                                                   @endforeach
                                                @endif
                                ...
                                <div class="col-lg-6 mt-3">
                                    <label  class="form-label">{{ __('shipping_charge') }} </label>
                                     <input type="text" name="shipping_charge" id="shipping_charge" class="form-control form--control" value="{{ old('shipping_charge') }}"/>
                                </div>
                                <div class="col-lg-6 mt-3">
                                    ...
                                    <select class="form-control form--control select2" name="shipping_status">
                                        ...
                                        @foreach (\Config::get('pos_default.shpping_status') as $key=>$shippingStatus)
                                            <option value="{{ $key }}">{{ __($shippingStatus) }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                    ...
                                    <div class="row">
                                        <div class="col-lg-6 mt-3">
                                            <div>
                                                ...
                                                <select class="form-control form--control select2" name="tax_id" id="applicableTax" data-url="{{ route('sale.taxrate.get') }}" > 
                                                ...
                                                <table class="table table-striped table-hover">
                                                    <thead>
                                                        <tr class="border-bottom bg-primary text-white head align-middle">
                                                            ...
                                                            <th class="text-left">: {{ businessCurrency(business_id()) }}<span class="text-white" id="total_shipping_charge"> {{ old('shipping_charge',0) }}</span></th> 
                                                        </tr> 
                                                        <tr class="border-bottom bg-primary text-white head align-middle">
    ...
    <input type="hidden" id="variation_location_url" data-url="{{ route('sale.variation.location.item') }}"/> 
    <input type="hidden" id="existing_customer" value="{{ \Modules\Customer\Enums\CustomerType::EXISTING_CUSTOMER }}" />
@endsection
 
@push('scripts')  
<script src="{{ static_asset('backend') }}/js/sell/sale/create.js"></script>  
    @if(old())
        <script src="{{ static_asset('backend') }}/js/sell/sale/create-old-value.js"></script>  
    @endif
@endpush