            ...
            <div class="row g-4">
                <div class="col-xl-12">
                    <div class="dashboard--widget ">
                        ...
                        <form action="{{ route('stock.transfer.update',['stock_t_id'=>$stockTransfer->id]) }}" method="post" enctype="multipart/form-data">
                            @csrf 
                            @method('put')
                            <div class="row mt-3">  
                                @if(business())
                                <div class="col-lg-6 mt-3">
                                   ...
                                   <select class="form-control form--control select2" name="branch_id" id="branch_id" >
                                        ...
                                        @foreach ($branches as $branch)
                                            <option value="{{ $branch->id }}" @if(old('branch_id',$stockTransfer->from_branch) == $branch->id) selected @endif>{{ @$branch->name }}</option>
                                        @endforeach
                                   </select>
                                    @error('branch_id')
                                   ...
                                   <select class="form-control form--control select2" name="to_branch"  >
                                        ...
                                        @foreach ($branches as $toBranch)
                                            @if(isUser() && Auth::user()->branch_id != $toBranch->id) 
                                                <option value="{{ $toBranch->id }}" @if(old('to_branch',$stockTransfer->to_branch) == $toBranch->id) selected @endif>{{ @$toBranch->name }}</option>
                                            @else
                                                <option value="{{ $toBranch->id }}" @if(old('to_branch',$stockTransfer->to_branch) == $toBranch->id) selected @endif>{{ @$toBranch->name }}</option>
                                            @endif
                                        @endforeach
                                        ...
                                        <table class="table table-striped table-hover text-left">
                                            ...
                                            <tbody id="stock_transfer_item_body">
                                                @foreach ($stockTransfer->TransferItems as $item) 
                                                    @include('stocktransfer::variation_location_item_edit')
                                                @endforeach
                                            </tbody>
                                            <thead>
                                                <tr class="border-bottom bg-primary text-white head align-middle ">
                                ...
                                <div class="col-lg-6 mt-3  ">
                                    <label for="applicable_tax" class="form-label">{{ __('shipping_charge') }} <span class="text-danger">*</span></label>
                                     <input type="text" name="shipping_charge" id="shipping_charge" class="form-control form--control" value="{{ $stockTransfer->shipping_charge }}"/>
                                        ...
                                        <table class="table table-striped table-hover">
                                            <thead>
                                                <tr class="border-bottom bg-primary text-white head align-middle">
                                                    ...
                                                    <th class="text-left">: {{ businessCurrency(business_id()) }}<span class="text-white" id="totlPurchaseTax"> {{ $stockTransfer->shipping_charge }}</span></th> 
                                                </tr>
                                                <tr class="border-bottom bg-primary text-white head align-middle">
                                                    <th class="text-left" width="50%">{{ __('total_amount') }}</th>
                                                    <th class="text-left">: {{ businessCurrency(business_id()) }}<span class="text-white" id="totlPurchasePriceWithTax"> {{ $stockTransfer->total_amount }}</span></th> 
                                                </tr>
                                                <input type="hidden" name="total_amount" id="total_amount" value="{{ $stockTransfer->total_amount }}"/>
                                            </thead> 
                                        </table>
                                    </div> 
                                </div>
