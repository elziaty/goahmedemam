            ...
            <div class="row g-4">
                ...
                <div class="col-xl-4">
                    <div class="dashboard--widget ">
                        <div class="d-flex">
                            ...
                            <span class="w-70">: {{ @$ticket->user->name }}</span>
                        </div>
                        <div class="d-flex">
                            <span class="w-30">{{ __('email') }}</span>
                            <span class="w-70">: {{ @$ticket->user->email }}</span>
                        </div>
                        <div class="d-flex">
                            <span class="w-30">{{ __('business_name') }}</span>
                            <span class="w-70">: {{ @$ticket->user->business->business_name }}</span>
                        </div>
                        <div class="d-flex">
                            <span class="w-30">{{ __('service') }}</span>
                            <span class="w-70">: {{ @$ticket->service->name }}</span>
                        </div>
                        <div class="d-flex">
                            <span class="w-30">{{ __('department') }}</span>
                            <span class="w-70">: {{ @$ticket->department->name }}</span>
                        </div>
                        <div class="d-flex">
                            <span class="w-30">{{ __('priority') }}</span>
                            <span class="w-70">: {{ @__($ticket->priority) }}</span>
                        </div>
                    </div>
                </div>
                ...
                <div class="col-xl-8">
                                ...
                                <div id="collapseOne" class="accordion-collapse collapse {{ $errors->any() == true? 'show':'' }}" data-bs-parent="#accordionExample">
                                    <div class="card-body p-0">
                                        <div class="accordion-body">
                                            <form  action="{{ route('ticket.reply',['support_id'=>$ticket->id,'reply'=>'reply']) }}" method="post" enctype="multipart/form-data">
                                                @csrf
                    ...
                    <div class="chat-box dashboard--widget">
                        <div class="chat-header d-flex justify-content-between align-items-center"> 
                            <div class="d-flex w-70 align-items-center">
                                <img src="{{ @$ticket->user->image }}" class="rounded-circle" height="50" width="50"/>
                                <div class="ml-10px">
                                    <p class="mb-0"><b>{{@$ticket->user->name }}</b></p>
                                    <span class="badge badge-pill badge-primary"> {{ @$ticket->user_type }}</span>
                                </div> 
                            </div> 
                             <span><small>{{ @$ticket->created_date_time }}</small></span>
                        </div>
                        <div class="chat-body mt-3">
                            {!! @$ticket->description !!}
                        </div>
                        @if(!empty($ticket->download_file))
                            <div class="chat-footer border-top mt-3">
                                 <a class="text-primary" href="{{ $ticket->download_file }}" download="">{{ __('download') }}</a>
                            </div>
                        @endif
                    </div> 
