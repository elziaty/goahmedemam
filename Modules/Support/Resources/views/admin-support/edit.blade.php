                ...
                <div class="col-xl-12">
                    <div class="dashboard--widget ">
                        <h4 class="card-title overflow-hidden"> 
                            ...
                            <a href="{{ route('ticket.index') }}" class="btn btn-primary float-right" data-bs-toggle="tooltip" title="{{ __('back') }}"
                        ...
                        <form action="{{ route('ticket.update',['id'=>$ticket->id]) }}" method="post" enctype="multipart/form-data">
                            ...
                            <div class="row mt-3">  
                                @if(isSuperadmin())
                                    <div class="{{ isSuperadmin() == true? 'col-lg-6':'col-lg-4' }}  mt-3">
                                        <label for="user_id" class="form-label">{{ __('business_user') }} <span class="text-danger">*</span></label>
                                        <select class="form-control form--control select2" id="user_id" name="user_id">
                                            <option selected disabled>{{ __('select') }} {{ __('business_user') }}</option>
                                            @foreach ($users as $user )
                                                <option value="{{ @$user->id }}" @if(old('user_id',@$ticket->user_id) == $user->id) selected @endif>{{ @$user->name }}</option>
                                            @endforeach
                                        </select>
                                        @error('user_id')
                                ...
                                <div class="{{ isSuperadmin() == true? 'col-lg-6':'col-lg-4' }} mt-3">
                                    ...
                                    <select class="form-control form--control select2" id="service_id" name="service_id">
                                        ...
                                        @foreach ($services as $service )
                                            <option value="{{ @$service->id }}" @if(old('service_id',@$ticket->service_id) == $service->id) selected @endif>{{ @$service->name }}</option>
                                        @endforeach
                                    </select>
                                    @error('service_id')
                                        <p class="text-danger pt-2">{{ $message }}</p>
                                ...
                                <div class="{{ isSuperadmin() == true? 'col-lg-6':'col-lg-4' }}  mt-3">
                                    ...
                                    <select class="form-control form--control select2" id="department_id" name="department_id">
                                        ...
                                        @foreach ($departments as $department )
                                            <option value="{{ @$department->id }}" @if(old('department_id',$ticket->department_id) == $department->id) selected @endif>{{ @$department->name }}</option>
                                        @endforeach
                                    </select>
                                    @error('department_id')
                                        <p class="text-danger pt-2">{{ $message }}</p>
                                    ...
                                    <select class="form-control form--control select2" id="priority" name="priority">
                                        <option selected disabled>{{ __('select') }} {{ __('priority') }}</option>  
                                           @foreach (\Config::get('pos_default.priority') as $key=>$priority ) 
                                                <option value="{{ @$priority }}" @if(old('priority',$ticket->priority) === $priority ) selected @endif>{{ __($priority) }}</option> 
                                           @endforeach
                                    </select>
                                    @error('priority')
                                        <p class="text-danger pt-2">{{ $message }}</p>
                                ...
                                <div class="col-lg-12 mt-3">
                                    ...
                                    <input type="text" name="subject" class="form-control form--control" id="subject" value="{{ old('subject',@$ticket->subject) }}">
                                    @error('subject')
                                        <p class="text-danger pt-2">{{ $message }}</p>
                                    @enderror
                                </div> 
                                ...
                                <div class="col-lg-12 mt-3">
                                    <label for="description" class="form-label">{{ __('description') }} <span class="text-danger">*</span></label>
                                     <textarea class="form-control form--control" name="description">{{ old('description',@$ticket->description) }}</textarea>
                                    @error('description')
                                        <p class="text-danger pt-2">{{ $message }}</p>
                                    @enderror
                                </div> 
