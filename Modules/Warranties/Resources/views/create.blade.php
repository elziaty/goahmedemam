...
<form action="{{ route('warranty.store') }}" method="post" enctype="multipart/form-data"  >
    ...
    <div class="row"> 
        ...
        <div class="col-lg-6 mt-3">
            ...
            <input type="text" name="duration" class="form-control form--control" required id="duration" value="{{ old('duration') }}">
            @error('duration')
                <p class="text-danger pt-2">{{ $message }}</p>
            @enderror
        </div>  
        ...
        <div class="col-lg-6 mt-3">
            <label for="duration_type" class="form-label">{{ __('duration_type') }} <span class="text-danger">*</span></label>
             <select class="form-control form--control select2" name="duration_type">
                <option value="{{ \Modules\Warranties\Enums\WarrantyType::DAY }}">   {{ __('day') }}   </option>
                <option value="{{ \Modules\Warranties\Enums\WarrantyType::MONTH }}"> {{ __('month') }} </option>
                <option value="{{ \Modules\Warranties\Enums\WarrantyType::YEAR }}">  {{ __('year') }}  </option>
             </select>
            @error('duration_type')
                <p class="text-danger pt-2">{{ $message }}</p>
            @enderror
        ...
        <div class="col-lg-12 mt-3">
            ...
            <textarea name="description" id="description" placeholder="{{ __('enter_description') }}" rows="5">{{ old('description') }}</textarea> 
            @error('description')
                <p class="text-danger pt-2">{{ $message }}</p>
            @enderror
        </div> 

        <div class="col-lg-6  mt-4 pt-lg-2">
            <div class="row">
                <div class="col-12">
