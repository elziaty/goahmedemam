            ...
            <div class="row g-4">
                <div class="col-xl-12">
                    <div class="dashboard--widget ">
                        ...
                        <form action="{{ route('hrm.leave.assign.update',['id'=>$leave_assign->id]) }}" method="post" enctype="multipart/form-data">
                            @csrf
                            @method('put')
                            <div class="row mt-3">
                                <div class="col-lg-6 mt-3">
                                    ...
                                    <select name="type_id" class="form-control form--control select2" id="type_id">
                                        ...
                                        @foreach ($leave_types as $leave_type)
                                            <option value="{{ $leave_type->id}}" @if(old('type_id',$leave_assign->type_id) == $leave_type->id) selected  @endif> {{ @$leave_type->name }}</option>
                                        @endforeach
                                    </select>
                                    @error('type_id')
                                ...
                                <div class="col-lg-6 mt-3">
                                    ...
                                    <select name="role_id" class="form-control form--control select2" id="role_id">
                                        ...
                                        @foreach ($roles as $role)
                                            <option value="{{ $role->id }}" @if(old('role_id',$leave_assign->role_id) == $role->id) selected  @endif>{{ @$role->name }}</option>
                                        @endforeach
                                    </select>
                                    @error('role_id')
                                        <p class="text-danger pt-2">{{ $message }}</p>
                                ...
                                <div class="col-lg-6 mt-3">
                                    ...
                                    <input type="text" value="{{ old('days',$leave_assign->days) }}" class="form-control form--control" name="days"/>
                                    @error('days')
                                        <p class="text-danger pt-2">{{ $message }}</p>
                                    @enderror
                                </div>
                                ...
                                <div class="col-lg-6 mt-4 pt-lg-3">
                                    <div class="d-flex mt-3">
                                        ...
                                        <input type="checkbox" class="status" id="status" switch="success" name="status" {{ old('status',$leave_assign->status) ==null? '':'checked' }} >
