...
class BranchController extends Controller
{
    ...
    {
        return $this->responseWithSuccess(__('success'), ['branches'=>  BranchResource::collection($this->repo->getBranch(business_id()))], 200);
    }
    ...
    public function store(Request $request)
    {
        ...
        if($this->repo->store($request)){
            return $this->responseWithSuccess(__('branch_added_successfully'), [], 200);
        }else{
    ...
    public function edit($id)
    {
        ...
        if($branch) {
            return $this->responseWithSuccess(__('success'), ['branch'=> new BranchResource($branch)], 200);
        }else {
            return $this->responseWithError(__('errors'), [], 500);
        }
    ...
    public function update(Request $request)
    {
        ...
        if($this->repo->update($request->id,$request)){
            return $this->responseWithSuccess(__('branch_updated_successfully'), [], 200);
        }else{
            return $this->responseWithError(__('errors'), [], 500);
        }
    }
    ...
    public function delete($id)
    {
        if($this->repo->delete($id)){
            return $this->responseWithSuccess(__('branch_deleted_successfully'), [], 200);
        }else{
            return $this->responseWithError(__('errors'), [], 500);
        }
    }
