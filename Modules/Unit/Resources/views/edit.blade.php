...
<form action="{{ route('units.update',['id'=>$unit->id]) }}" method="post" enctype="multipart/form-data"  >
    @csrf
    @method('put')
    <div class="row"> 
        @if(isSuperadmin())
                ...
                <select class=" form-control form--control select2" name="business_id" required id="business_id"  >
                    <option  disabled selected>{{ __('select') }} {{ __('business') }}</option> 
                        @foreach ($businesses as $business )
                            <option  value="{{ $business->id }}" @if(old('business_id',$unit->business_id) == $business->id) selected @endif>{{  @$business->business_name }}</option>
                        @endforeach 
                </select>
                @error('business_id')
        ...
        <div class="col-lg-6 mt-3">
            ...
            <input type="text" name="name" class="form-control form--control" required id="name" value="{{ old('name',$unit->name) }}">
            @error('name')
                <p class="text-danger pt-2">{{ $message }}</p>
            @enderror
        </div>   
        <div class="col-lg-6 mt-3">
            ...
            <input type="text" name="short_name" class="form-control form--control" required id="short_name" value="{{ old('short_name',$unit->short_name) }}">
            @error('short_name')
                <p class="text-danger pt-2">{{ $message }}</p>
            @enderror
        </div>  
        ...
        <div class="col-lg-6 mt-3">
            ...
            <input type="text" name="position" class="form-control form--control" id="position" value="{{ old('position',$unit->position) }}">
            @error('position')
                <p class="text-danger pt-2">{{ $message }}</p>
            @enderror
        </div>
            ...
            <div class="row">
                <div class="col-12">
                    <div class="d-flex mt-3">
                        ...
                        <input type="checkbox" class="status" id="status" switch="success" name="status" {{ old('status',$unit->status) == \App\Enums\Status::INACTIVE ? '':'checked' }} >
