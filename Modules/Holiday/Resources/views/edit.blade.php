            ...
            <div class="row g-4">
                <div class="col-xl-12">
                    <div class="dashboard--widget ">
                        ...
                        <form action="{{ route('hrm.attendance.holiday.update',['id'=>$holiday->id]) }}" method="post" enctype="multipart/form-data">
                            @csrf
                            @method('put')
                            <div class="row mt-3">
                                ...
                                <div class="col-lg-6 mt-3">
                                    ...
                                    <input type="text" name="name" class="form-control form--control" id="name" value="{{ old('name',@$holiday->name) }}">
                                    @error('name')
                                ...
                                <div class="col-lg-6  mt-3">
                                    ...
                                    <input type="text" readonly name="from_date" class="form-control form--control dateformat2" id="from" value="{{ old('from_date',\Carbon\Carbon::parse(@$holiday->from)->format('d-m-Y')) }}">
                                    @error('from_date')
                                        <p class="text-danger pt-2">{{ $message }}</p>
                                    @enderror
                                </div>
                                <div class="col-lg-6  mt-3">
                                    ...
                                    <input type="text" readonly name="to_date" class="form-control form--control dateformat2" id="to" value="{{ old('to_date',\Carbon\Carbon::parse(@$holiday->to)->format('d-m-Y')) }}">
                                    @error('to_date')
                                        <p class="text-danger pt-2">{{ $message }}</p>
                                    @enderror
                                </div>
                                ...
                                <div class="col-lg-2 mt-4 pt-lg-3">
                                    <div class="d-flex mt-3">
                                        ...
                                        <input type="checkbox" class="status" id="status" switch="success" name="status" {{ old('status',@$holiday->status) == \App\Enums\Status::INACTIVE? '':'checked' }} >
                                ...
                                <div class="col-lg-12  mt-3">
                                    <label for="to" class="form-label">{{ __('note') }} </label>
                                     <textarea id="note" class="form-control form--control" name="note">{{ old('note',@$holiday->note) }}</textarea>
                                </div>
