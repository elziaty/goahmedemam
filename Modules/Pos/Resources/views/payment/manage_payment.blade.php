<div class="row purchase_details_modal border-bottom py-2">
    ...
    <div class="col-lg-4">   
        ...
        <div  >
            <span>{{ __('total_sale_amount') }}</span>: {{ businessCurrency($pos->business_id) }} {{ number_format($pos->total_sale_price,2) }}
        </div>
        <div >
        ...
        <table class="table table-striped table-hover">
            <thead>
                <tr class="border-bottom bg-primary text-white head align-middle">
                    ...
                    @if(hasPermission('pos_update_payment') || hasPermission('pos_delete_payment')) 
                    <th>{{ __('action') }} </th> 
                    @endif
                </tr>
            </thead>
            <tbody > 
                @foreach ($pos->payments as $payment)                    
                    <tr>
                        ...
                        <td data-title="{{ __('return_no') }}">{{  $payment->pos->invoice_no}}</td>
                        ...
                        <td data-title="{{ __('amount') }}">{{ @businessCurrency($payment->pos->business_id) }} {{ @$payment->amount }}</td> 
                        ...
                        <td data-title="{{ __('action') }}">
                            @if(hasPermission('pos_update_payment')) 
                                <a href="#"  class="edit_payment px-3" data-url="{{ route('pos.edit.payment',$payment->id) }}"><i class="fa fa-pen"></i></a>
                            @endif
                            @if(hasPermission('pos_delete_payment')) 
                                <a href="{{ route('pos.delete.payment',$payment->id) }}"  class="px-3"  ><i class="fa fa-trash"></i></a>
                            @endif
                        </td>
                        @endif 
                    </tr>
...
@if(hasPermission('pos_add_payment'))
<div id="payment_form"> 
    <h5 class="mt-3">{{ __('add_payment') }}</h5>
    <form action="{{ route('pos.add.payment',['pos_id'=>$pos->id]) }}" method="post" enctype="multipart/form-data">
        @csrf
        <div class="row">   
            ...
            <div class="col-lg-6 mt-2 ">
                ...
                <input type="text" name="amount" class="form-control form--control" value="{{ $pos->dueAmount }}" required/>
            </div>  
            <div class="col-lg-6 mt-2 ">
