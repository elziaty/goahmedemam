...
@section('maincontent') 
                            ...
                            <div class="row"> 
                                   ...
                                   <div class="input-group">
                                       <select class="form-control form--control select2" name="customer_id" id="customer" data-url="{{ route('customers.get.customer') }}" data-customerphone="{{ __('customer_phone') }}" data-customeraddress="{{ __('customer_address') }}" >
                                           ...
                                           @foreach ($customers as $customer)
                                               <option value="{{ @$customer->id }}">{{ @$customer->name }} | {{ __('balance') }}: {{ $customer->balance }}</option>
                                           @endforeach
                                       </select>
                                        ...
                                        {{-- variation location item will appends --}}
                                         @if (old('variation_locations'))
                                            @foreach (old('variation_locations') as $item ) 
                                                @include('pos::variation_location_item_old',['item'=>$item])
                                            @endforeach
                                         @endif
                                     </tbody> 
                                     <thead>
                             ...
                             <div class="row">
                                 <div class="col-lg-6 mt-3">
                                     ...
                                     <textarea class=" form--control" name="shipping_details" >  </textarea>
                                 </div>
                                 <div class="col-lg-6 mt-3">
                                 ...
                                 <div class="col-lg-6 mt-3">
                                     ...
                                     <select class="form-control form--control select2" name="shipping_status">
                                         ...
                                         @foreach (\Config::get('pos_default.shpping_status') as $key=>$shippingStatus)
                                             <option value="{{ $key }}" @if(old('shipping_status') == $key) selected @endif>{{ __($shippingStatus) }}</option>
                                         @endforeach
                                     </select>
                                 </div> 
                                 <div class="col-lg-6 mt-3">
                                 ...
                                 <table class="table table-striped table-hover">
                                     <thead>
                                         <tr class="border-bottom bg-primary text-white head align-middle">
                                             ...
                                             <th class="text-left">: {{ businessCurrency(business_id()) }}<span class="text-white" id="total_shipping_charge"> 0</span></th> 
                                         </tr>
                             ...
                             <div class="row mt-3">
                                <div class="col-12 text-center">
                                    ...
                                    @if(Session::has('pos'))
                                        <button type="button"  class="btn btn-danger m-2" onclick="window.print()">{{ __('print') }}</button>
                                    @endif
                                </div>
                             </div>
                         </form> 
    ...
    {{-- print --}}
    @if(Session::has('pos'))
        @include('pos::sale_print',['pos'=>Session::get('pos')])
    @endif 
    {{-- end print --}}
...
@endsection  
@push('styles')
    <link rel="stylesheet" href="{{static_asset('backend')}}/css/pos/print.css"> 
@endpush 
@push('scripts')
    <script src="{{ static_asset('backend') }}/js/pos/pos.js"></script>
    <script src="{{ static_asset('backend') }}/js/pos/create-old-value.js"></script>
