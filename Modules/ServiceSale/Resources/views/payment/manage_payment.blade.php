        ...
        <table class="table table-striped table-hover">
            <thead>
                <tr class="border-bottom bg-primary text-white head align-middle">
                    ...
                    @if(hasPermission('service_sale_update_payment') || hasPermission('service_sale_delete_payment')) 
                    <th>{{ __('action') }} </th> 
                    @endif
                </tr>
            </thead>
            <tbody > 
                @foreach ($sale->payments as $payment)                    
                    <tr>
                        ...
                        @if(hasPermission('sale_update_payment') || hasPermission('service_sale_delete_payment')) 
                        <td data-title="{{ __('action') }}">
                            @if(hasPermission('sale_update_payment')) 
                                <a href="#"  class="edit_payment px-3" data-url="{{ route('servicesale.edit.payment',$payment->id) }}"><i class="fa fa-pen"></i></a>
                            @endif
                            @if(hasPermission('service_sale_delete_payment')) 
                                <a href="{{ route('servicesale.delete.payment',$payment->id) }}"  class="px-3"  ><i class="fa fa-trash"></i></a>
                            @endif
                        </td>
                        @endif 
                    </tr>
...
@if(hasPermission('service_sale_add_payment'))
<div id="payment_form"> 
    <h5 class="mt-3">{{ __('add_payment') }}</h5>
    <form action="{{ route('servicesale.add.payment',['service_sale_id'=>$sale->id]) }}" method="post">
        @csrf
        <div class="row">   
