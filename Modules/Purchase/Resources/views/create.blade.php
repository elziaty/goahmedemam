                                    ...
                                    <div class="input-group"> 
                                        <select class="form-control form--control select2" name="supplier_id" >
                                                ...
                                                @foreach ($suppliers as $supplier)
                                                    <option value="{{ $supplier->id }}" @if(old('supplier_id') == $supplier->id) selected @endif>{{ @$supplier->name }} @if(!empty($supplier->company_name)) ({{ @$supplier->company_name }}) @endif</option>
                                                @endforeach
                                        </select>
                                ...
                                <div class="col-lg-6 mt-3">
                                   ...
                                   <select class="form-control form--control select2" name="purchase_status" id="purchase_status" >
                                        ...
                                        @foreach (\Config::get('pos_default.purchase.purchase_status') as $key=>$purchaseStatus)
                                            <option value="{{ $key }}" @if(old('purchase_status') == $key) selected @endif>{{@__($purchaseStatus) }}</option>
                                        @endforeach
                                   </select>
                                            ...
                                            <tbody id="purchase_item_body"> 
                                                @if (old('variation_locations'))
                                                    @foreach (old('variation_locations') as $item)
                                                        @include('purchase::variation_location_item_old',['item'=>$item])
                                                    @endforeach
                                                @endif
...
@endsection
@push('scripts')
    <script src="{{ static_asset('backend') }}/js/purchase/create.js"></script> 
@endpush