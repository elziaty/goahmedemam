<div class="row purchase_details_modal border-bottom py-2">
    ...
    <div class="col-lg-4">  
        ...
        <div  >
            <span>{{ __('total_purchase_cost') }}</span>: {{ businessCurrency($purchase->business_id) }} {{ number_format($purchase->total_purchase_cost,2) }}
        </div>
        <div >
            <span>{{ __('payment_status') }}</span>: {!! $purchase->my_payment_status !!}
        </div>
        <div class="text-primary">
            <span><b>{{ __('total_amount') }}</span>: {{ businessCurrency($purchase->business_id) }} {{ number_format($purchase->dueAmount,2) }}</b>
        </div>
    </div>
</div>
        ...
        <table class="table table-striped table-hover">
            <thead>
                <tr class="border-bottom bg-primary text-white head align-middle">
                    ...
                    @if(hasPermission('purchase_update_payment') || hasPermission('purchase_delete_payment')) 
                    <th>{{ __('action') }} </th> 
                    @endif
                </tr>
            </thead>
            <tbody > 
                @foreach ($purchase->payments as $payment)                    
                    <tr>
                        ...
                        <td data-title="{{ __('purchase_no') }}">{{  $payment->purchase->purchase_no}}</td>
                        ...
                        <td data-title="{{ __('amount') }}">{{ @businessCurrency($payment->purchase->business_id) }} {{ @$payment->amount }}</td> 
                        ...
                        <td data-title="{{ __('action') }}">
                            @if(hasPermission('purchase_update_payment') ) 
                            <a href="#"  class="edit_payment px-3" data-url="{{ route('purchase.edit.payment',$payment->id) }}"><i class="fa fa-pen"></i></a>
                            @endif
                            @if(hasPermission('purchase_delete_payment')) 
                            <a class="px-3" href="{{ route('purchase.delete.payment',$payment->id) }}"   ><i class="fa fa-trash"></i></a>
                            @endif
                        </td> 
                        @endif
                    </tr>
...
@if(hasPermission('purchase_add_payment'))
<div id="payment_form"> 
    <h5 class="mt-3">{{ __('add_payment') }}</h5>
    <form action="{{ route('purchase.add.payment',['purchase_id'=>$purchase->id]) }}" method="post" enctype="multipart/form-data">
        @csrf
        <div class="row">   
            ...
            <div class="col-lg-6 mt-2 ">
                ...
                <input type="text" name="amount" class="form-control form--control" value="{{ $purchase->dueAmount }}" required/>
            </div>  
            <div class="col-lg-6 mt-2 ">
