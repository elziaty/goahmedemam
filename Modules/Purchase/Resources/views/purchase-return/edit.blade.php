            ...
            <div class="row g-4">
                <div class="col-xl-12">
                    <div class="dashboard--widget ">
                        ...
                        <form action="{{ route('purchase.return.update',['purchase_return_id'=>$purchase_return->id]) }}" method="post" enctype="multipart/form-data">
                            @csrf 
                            @method('put')
                            <div class="row mt-3">  
                                   ...
                                   <div class="input-group">
                                       ...
                                       <select class="form-control form--control select2" name="supplier_id" >
                                            ...
                                            @foreach ($suppliers as $supplier)
                                                <option value="{{ $supplier->id }}" @if(old('supplier_id',$purchase_return->supplier_id) == $supplier->id) selected @endif>{{ @$supplier->name }} ({{ @$supplier->company_name }})</option>
                                            @endforeach
                                       </select>
                                    ...
                                    <div class="table-responsive   category-table product-view-table mt-2">
                                        <table class="table table-striped table-hover">
                                            ...
                                            <tbody id="purchase_item_body"> 
                                                ...
                                                @foreach ($purchase_return->purchaseReturnItems as $return_item)
                                                    @include('purchase::purchase-return.variation_location_item_edit')
                                                @endforeach
                                            </tbody>
                                            <thead>
                                                <tr class="border-bottom bg-primary text-white head align-middle ">
                                ...
                                <div class="col-lg-6 mt-3  ">
                                    ...
                                    <select class="form-control form--control select2" name="tax_id" id="applicableTax" data-url="{{ route('purchase.taxrate.get') }}" > 
                                        ...
                                        @foreach ($taxRates as $tax)
                                            <option value="{{ $tax->id }}" @if(old('tax_id',$purchase_return->tax_id) == $tax->id) selected @endif>{{ @$tax->name }}</option>
                                        @endforeach 
                                    </select>
                                    @error('tax_id')
                                        <p class="text-danger pt-2">{{ $message }}</p>
                                    ...
                                    <div class="table-responsive   category-table product-view-table mt-3">
                                        <table class="table table-striped table-hover">
                                            <thead>
                                                ...
                                                <input type="hidden" name="total_price" id="total_price" value="{{ $purchase_return->total_price }}"/>
                                                <input type="hidden" name="total_tax_amount" id="total_tax_amount" value="{{ $purchase_return->total_tax_amount }}"/>
                                                <input type="hidden" name="total_buy_cost" id="total_buy_cost" value="{{ $purchase_return->total_buy_cost }}"/>
...
@endsection
@push('scripts')
    <script src="{{ static_asset('backend') }}/js/purchase/purchase_return/edit.js"></script> 
@endpush